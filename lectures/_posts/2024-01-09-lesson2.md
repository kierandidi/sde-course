...
If we assume this discretisation, the transition density equality we wrote down above is not valid anymore except for the limit $$\delta \rightarrow 0$$. In this approximation, the transition densities are Gaussians that depend on the drift and diffusion term in the following way:

$$
\begin{align}
p_{t+\delta \mid t}(y \mid x) &= \mathcal{N}(y \mid x + f^+(x), \delta \sigma^2(x))
p_{t \mid t+\delta}(x_t \mid x_{t+\delta}) &= \mathcal{N}(x \mid y + f^-(y), \delta \sigma^2(y))
\end{align}
$$

where we denote the drift term at time $$t$$ as $$f^+(x) = \mu(x, t)$$, the drift term at time $$t+\delta$$ as $$f^-(y) = - \mu(y, t)$$ and ignore the time dependency of the drift and diffusion terms for simplicity.

Let us now insert these equations into the chain rule equality we wrote down above and see what happens (the equalities hold only for the limit $$\delta \rightarrow 0$$, indicated by the delta above the equality sign):

$$
\begin{align}
p_{t+\delta \mid t}(y \mid x) p_t(x) &=^{\delta} p_{t \mid t+\delta}(x \mid y) p_{t+\delta}(y) \hspace{10px} \mid \text{insert Gaussians} \\
\mathcal{N}(y \mid x + f^+(x), \delta \sigma^2(x)) p_t(x) &= \mathcal{N}(x \mid y + f^-(y), \delta \sigma^2(y)) p_{t+\delta}(y) \hspace{10px} \mid \text{take log}\\
\log \mathcal{N}(y \mid x + f^+(x), \delta \sigma^2(x)) + \log p_t(x) &= \log \mathcal{N}(x \mid y + f^-(y), \delta \sigma^2(y)) + \log p_{t+\delta}(y) \hspace{10px} \mid \text{expand log}\\
-\frac{1}{2} \log 2 \pi \delta \sigma^2(x) - \frac{1}{2} \frac{(y - x - f^+(x))^2}{\delta \sigma^2(x)} + \log p_t(x) &= -\frac{1}{2} \log 2 \pi \delta \sigma^2(y) - \frac{1}{2} \frac{(x - y - f^-(y))^2}{\delta \sigma^2(y)} + \log p_{t+\delta}(y) \hspace{10px} \mid \text{rearrange}\\
\log p_{t+\delta}(y) - \log p_t(x) &= \frac{1}{2} \log \frac{\sigma^2(x)}{\sigma^2(y)} - \frac{1}{2} \frac{(x - y - f^-(y))^2}{\delta \sigma^2(y)} + \frac{1}{2} \frac{(y - x - f^+(x))^2}{\delta \sigma^2(x)} \hspace{10px} \mid \text{take limit } \delta \rightarrow 0\\
