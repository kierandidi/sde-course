...
Let us now insert these equations into the chain rule equality we wrote down above and see what happens:

$$
\begin{align}
p_{t+\delta \mid t}(y \mid x) p_t(x) &= p_{t \mid t+\delta}(x \mid y) p_{t+\delta}(y) \hspace{10px} \mid \text{insert Gaussians} \\
