...
> where $$Z_T \sim \delta_z \text{ and } p^h_{T \mid t}(z \mid Z_t) =  p^h_{T \mid t}(z_T = z\midz_t = Z_t)$$  is called the *h-transform*.
{:.lead}
...
&= \frac{{\color{red}p_{T \mid t+\delta}(z_T=z \mid z_{t+\delta}) }p_{t+\delta \mid t}(z_{t+\delta}\mid z_{t}) }{{\color{red}p_{T \mid t}(z_T=z \mid z_{t})}}
\end{align}
$$
...
$$
\begin{align}
&\partial_t p_{t\mid s, T}(z_{t} \mid z_{s}, z_T=z) = \frac{1 }{{\color{red}p_{T \mid s}(z_T=z \mid z_{s})}}\partial_t {\color{red}p_{T \mid t}(z_T=z \mid z_{t}) }p_{t\mid s}(z_{t}\mid z_{s}) /hspace{10px} \mid \text{product rule} \\
&= \frac{1 }{{\color{red}p_{T \mid s}(z_T=z \mid z_{s})}} ( p_{t\mid s}(z_{t}\mid z_{s}) \partial_t {\color{red}p_{T \mid t}(z_T=z \mid z_{t}) } + {\color{red}p_{T \mid t}(z_T=z \mid z_{t}) }\partial_t p_{t\mid s}(z_{t}\mid z_{s}) ) \hspace{10px} \mid \text{Def. h-transform} \\
&= \frac{1 }{{\color{red}h(z_s,s)}} ( -p_{t\mid s}(z_{t}\mid z_{s}) \mathcal{P}^{\dagger} {\color{red}h(z_t,t) } + {\color{red}h(z_t,t) }\mathcal{P}^{} p_{t\mid s}(z_{t}\mid z_{s}) ) \hspace{10px} \mid \text{regrouping} \\
&= \frac{1 }{{\color{red}h(z_s,s)}} \left(\mathcal{P}^{} {\color{red}h(z_t,t) }p_{t\mid s}(z_{t}\mid z_{s})  + \nabla p_{t\mid s}(z_{t}\mid z_{s}) \cdot \nabla  {\color{red}h(z_t,t) }   + p_{t\mid s}(z_{t}\mid z_{s})  \Delta   {\color{red}h(z_t,t) } \right) \hspace{10px} \mid \text{log-deriv. trick} \\
&= \frac{1 }{{\color{red}h(z_s,s)}} \left(\mathcal{P}^{} {\color{red}h(z_t,t) }p_{t\mid s}(z_{t}\mid z_{s})  + \nabla \cdot (h(z_t,t) p_{t\mid s}(z_{t}\mid z_{s}) {\color{red}\nabla \ln h(z_t,t) } )\right)
\end{align}
$$
