...
> where $$Z_T \sim \delta_z \text{ and } p^h_{T \mid t}(z \mid Z_t) =  p^h_{T|t}(z_T = z|z_t = Z_t)$$  is called the *h-transform*.
{:.lead}
...
$$
\begin{align}
p_{t+\delta | t, T}(z_{t+\delta} | z_{t}, z_T=z) &= \frac{p_{T | t, t+\delta}(z_T=z | z_{t}, z_{t+\delta}) p_{t+\delta | t}(z_{t+\delta}| z_{t}) }{p_{T | t}(z_T=z | z_{t})} \hspace{10px} \mid \text{Markov} \\
&= \frac{{\color{red}p_{T | t+\delta}(z_T=z | z_{t+\delta}) }p_{t+\delta | t}(z_{t+\delta}| z_{t}) }{{\color{red}p_{T | t}(z_T=z | z_{t})}}
\end{align}
$$

$$
\begin{align}
&\partial_t p_{t| s, T}(z_{t} | z_{s}, z_T=z) = \frac{1 }{{\color{red}p_{T | s}(z_T=z | z_{s})}}\partial_t {\color{red}p_{T | t}(z_T=z | z_{t}) }p_{t| s}(z_{t}| z_{s}) /hspace{10px} \mid \text{product rule} \\
&= \frac{1 }{{\color{red}p_{T | s}(z_T=z | z_{s})}} ( p_{t| s}(z_{t}| z_{s}) \partial_t {\color{red}p_{T | t}(z_T=z | z_{t}) } + {\color{red}p_{T | t}(z_T=z | z_{t}) }\partial_t p_{t| s}(z_{t}| z_{s}) ) \hspace{10px} \mid \text{Def. h-transform} \\
&= \frac{1 }{{\color{red}h(z_s,s)}} ( -p_{t| s}(z_{t}| z_{s}) \mathcal{P}^{\dagger} {\color{red}h(z_t,t) } + {\color{red}h(z_t,t) }\mathcal{P}^{} p_{t| s}(z_{t}| z_{s}) ) \hspace{10px} \mid \text{regrouping} \\
&= \frac{1 }{{\color{red}h(z_s,s)}} \left(\mathcal{P}^{} {\color{red}h(z_t,t) }p_{t| s}(z_{t}| z_{s})  + \nabla p_{t| s}(z_{t}| z_{s}) \cdot \nabla  {\color{red}h(z_t,t) }   + p_{t| s}(z_{t}| z_{s})  \Delta   {\color{red}h(z_t,t) } \right) \hspace{10px} \mid \text{log-deriv. trick} \\
&= \frac{1 }{{\color{red}h(z_s,s)}} \left(\mathcal{P}^{} {\color{red}h(z_t,t) }p_{t| s}(z_{t}| z_{s})  + \nabla \cdot (h(z_t,t) p_{t| s}(z_{t}| z_{s}) {\color{red}\nabla \ln h(z_t,t) } )\right)
\end{align}
$$
