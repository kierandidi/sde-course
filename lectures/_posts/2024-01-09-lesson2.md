...
We can now use $$\tau = 1-t$$ as time reversal to get a FPE for the reverse SDE:

$$-\partial_t p(x_t) = \partial_\tau p(x_{1-\tau}) = \partial_{x_{1-\tau}} \Big[ p(x_{1-\tau}) \left( \mu(x_{1-\tau}) + \frac{1}{p(x_{1-\tau})} \partial_{x_{1-\tau}} \left[ \sigma^2(x_{1-\tau}) \ p(x_{1-\tau}) \right] \right) \Big] + \frac{1}{2} \partial_{x_{1-\tau}}^2 \left[ p( x_{1-\tau}) \sigma^2(x_{1-\tau}) \right]$$

We can now use this FPE to get the corresponding reverse SDE that can be solved backward in time:

$$dX_t = \big[ - \mu(x_{1-\tau}) + \frac{1}{p(x_{1-\tau})} \partial_{x_{1-\tau}} \left[ \sigma^2(x_{1-\tau}) \ p(x_{1-\tau}) \right] \big] d\tau + \sigma(x_{1-\tau}) d\tilde{W}_t$$

where $$\tilde{W}_t$$ is a reverse Brownian motion that flows backward in time.

We can again simplify this by assuming a scalar diffusion term and no dependency on $$x_{1-\tau}$$ to get

$$
\begin{align}
dX_t = \big[ - \mu(x_{1-\tau}) + \frac{1}{p(x_{1-\tau})} \sigma^2 \partial_{x_{1-\tau}} \log p(x_{1-\tau}) \big] d\tau + \sigma(x_{1-\tau}) d\tilde{W}_t \hspace{10px} \mid \text{cancel }p(x_{1-\tau})\\
dX_t = \big[ - \mu(x_{1-\tau}) + \sigma^2 \partial_{x_{1-\tau}} \log p(x_{1-\tau}) \big] d\tau + \sigma(x_{1-\tau}) d\tilde{W}_t \hspace{10px} \mid \text{log-derivative trick}\\
dX_t = \big[ - \mu(x_{1-\tau}) + \sigma^2 \partial_{x_{1-\tau}} \log p(x_{1-\tau}) \big] d\tau + \sigma(x_{1-\tau}) d\tilde{W}_t \hspace{10px} \mid \text{reverse time}\\
\end{align}
$$

We can see that similarly to our heuristic derivation, the reverse SDE contains the score term $$\partial_{x_{1-\tau}} \log p(x_{1-\tau})$$, but with the opposite sign. This is not a coincidence, but a direct consequence of the fact that the SDE and FPE have the same marginal probability densities. Let us look at the derivation for this.

### 2.3 Time Reversal in score-based models

How do we use this in score-based models? 
...
You saw that we used Langevin sampling-like techniques to simulate our reverse SDE. In [Song et al](), it was shown that you do not necessarily need to do that; instead of modelling a stochastic process, we can replace this backward diffusion process with its corresponding *deterministic process* (i.e. replace an SDE with an ODE) and still have the same marginal probability densities $$\{p_t(x)\}_{t=0}^T$$. This is known as the *probability flow ODE*.
...
$$\frac{\partial}{\partial t} p_t(x) = - \frac{\partial}{\partial x} \Big[ \mu(x,t)- \frac{1}{2} \nabla_x [\sigma(x,t)\sigma(x,t)^T] - \frac{1}{2}\sigma(x,t)\sigma(x,t)^T \nabla_x [\log p_t(x)]\Big]$$

If we now define the whole right hand side as $$\tilde{\mu}(x,t)p_t(x)$$, we get
