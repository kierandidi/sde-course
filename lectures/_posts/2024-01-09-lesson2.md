...
Image from [this talk](https://www.youtube.com/watch?v=5ZSwYogAxYg) by Yaron Lipman. We can formulate score matching in terms of conditional transition densities for which we have closed-form solutions. We can then use these closed-form solutions to directly "jump" from data samples to an arbitrary time point $$t$$.
{:.figcaption}

"*Wait*", you may say at this point, "*the score matching loss involving the conditional properties in this picture is different to the one we talked about before! How do I know they actually are the same?*" You are right, this is not directly obvious! There is no reason to believe *a priori* that minimising $$\mathbb{E} \left[\left\lVert \nabla_x \ln p_{t \mid 0}(X_t \mid X_0)- s_{\theta}(t, X_t)\right\rVert^2 \right]$$ is the same as minimising $$\mathbb{E}_{t \in \mathcal{U}(0,T)}\mathbb{E}_{p_t(\mathbf{x})}[\left\lVert \nabla_\mathbf{x} \log p_t(\mathbf{x},t) - \mathbf{s}_\theta(\mathbf{x},t) \right\rVert_2^2]$$.
...
$$
\begin{align}
f^{*}(Y) = \argmin_{f-\mathrm{is\; measurable}} \mathbb{E}[(X- f(Y))^2]
\end{align}
$$
...
$$
\begin{align}
f^{*}(t,x)&=\argmin_{s-\mathrm{is\; measurable}} \mathbb{E}[(X_{t+\delta}- f(X_t))^2] //
...
&= \frac{1}{p_t(x)}\int p_0(x_0) \nabla_x p_{t \mid 0}(x \mid x_0) \mathrm{d}x_0 \hspace{10px} \mid \text{dom. convergence theorem}\\
...
![cnf](/assets/img/lessons/normalising_flows.png)

Image from [this talk](https://www.youtube.com/watch?v=5ZSwYogAxYg) by Yaron Lipman.
{:.figcaption}
...
Image from [the stochastic interpolant paper](https://arxiv.org/abs/2303.08797).
{:.figcaption}
...
Let us consider the time reversed SDE $$dX_t = \f^-(X_t) dt + \sigma_t d\tild{W}_t$$ and the general hard constraint $$(X_0 \in B)$$ ($$X_0$$ since we consider the endpoint of a reversed SDE running from $$T$$ to $$0$$). We can then use Doob's h-transform to get the conditioned SDE:

$$
\begin{align}
dZ_t &= f^-_{cond}(Z_t)dt + \sigma_t d\tild{W}_t \\
	 &= [f^-(Z_t) + \sigma^2 \nabla_{Z_t}p^h_{0 \mid t}(X_0 \in B \mid Z_t)]dt + \sigma_t d\tild{W}_t
\end{align}
$$
...
\begin{align}
dZ_t &= f^-_{cond}(Z_t)dt + \sigma_t d\tild{W}_t \\
	 &= [f^-(Z_t) + \sigma^2 \nabla_{Z_t}p^h_{0 \mid t}(\mathcal{A}(X_0) = y \mid Z_t)]dt + \sigma_t d\tild{W}_t
\end{align}
$$
...
\begin{align}
dZ_t &= f^-_{cond}(Z_t)dt + \sigma_t d\tild{W}_t \\
	 &= [f^-(Z_t) + \sigma^2 \nabla_{Z_t} \left\lVert y - A \mathbb{E}[X_0 \mid X_t = Z_t0] \right\rVert]^2_{\gamma_t}dt + \sigma_t d\tild{W}_t
\end{align}
$$
...
![reconstruction_guidance](/assets/img/lessons/reconstruction_guidance.png)ß
