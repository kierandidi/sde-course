...
For the example of the backward SDE $$dX_t = [\mu(X_t, t) - \sigma^2(t) \nabla_x \log p_t(X_t)]dt + \sigma(t)d\bar{W}_t$$ (where we assume a scalardiffusion term with no dependency on $$X_t$$), the corresponding ODE is given by
...
Let's start again with our general SDE $$dX_t = \mu(Xx, t)dt + \sigma(x, t)dW_t$$, where we assume $$\sigma(x, t)$$ to be matrix-valued and dependent on $$x$$. We know that the Focker-Planck Equation of this general SDE is given by

$$\frac{\partial}{\partial t} p_t(x) = - \frac{\partial}{\partial x}[\mu(x,t)p_t(x)] + \frac{\partial^2}{\partial x^2}[\frac{\sigma^2(X_t,t)}{2}p_t(x)]$$
...
$$\frac{\partial}{\partial t} p_t(x) = - \frac{\partial}{\partial x}[\mu(x,t)p_t(x)] + \frac{1}{2} \frac{\partial^2}{\partial x^2}[\sigma^2(x,t)p_t(x)] + \frac{1}{2} \frac{\partial}{\partial x}[\sigma^2(x,t)\frac{\partial p_t(x)}{\partial x}]$$

Pulling the $$p_t(x)$$ out of the second term and pulling the $$\frac{\partial}{\artial x}$$ outside the whole expression, we get

$$\frac{\partial}{\partial t} p_t(x) = - \frac{\partial}{\partial x}[\mu(x,t)p_t(x)] + \frac{1}{2} \frac{\partial}{\partial x}[\sigma^2(x,t)\frac{\partial p_t(x)}{\partial x}] + \frac{1}{2} \sigma^2(x,t) \frac{\partial^2 p_t(x)}{\partial x^2}$$

If we now define the whole right hand side 
